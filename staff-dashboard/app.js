...
if (!localStorage.getItem('loggedInPhone') && !window.location.pathname.includes('login.html') && !window.location.pathname.includes('register.html')) {
...
async function loadServiceCatalog() {
  ...
  try {
    ...
    const resCli = await fetch("http://localhost:5000/api/clients");
    if (resSvc.ok) {
...
async function syncLiveAppointments() {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/my-appointments?staffId=${staffId}`);
    if (response.ok) {
...
async function buildLeave(s) {
  ...
  try {
      const res = await fetch(`http://localhost:5000/api/my-leaves?staffId=${staffId}`);
      if (res.ok) {
...
async function submitLeave() {
  ...
  const staff = STAFF_DATA ? STAFF_DATA[currentStaff] : null;
  const staffId = localStorage.getItem('loggedInStaffId') || (staff ? staff.id : 'staff1');
  const staffName = localStorage.getItem('loggedInUser') || (staff ? staff.name : 'Staff Member');

  const newLeave = { staffId, staffName, type: leaveType, fromDate: from, toDate: to, reason, status: "Pending" };
  ...
  try {
      ...
      // Fallback to local
      const localLeave = { type: leaveType, from, to, reason, status: "pending" };
      if (staffLeaves) {
        staffLeaves.unshift(localLeave);
        saveStaffLeaves();
      }
  }
...
function setSection(sec, btn) {
  ...
  const label = btn.textContent.trim().replace(/[^a-zA-Z\s]/g, "").trim() || sec;
